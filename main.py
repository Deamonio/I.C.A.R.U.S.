...
class Config:
    ...
    PORT = '/dev/ttyUSB0'
    BAUD_RATE = 115200
    SCREEN_WIDTH = 1100  # 더 넓은 화면
    SCREEN_HEIGHT = 700
    
    KEY_REPEAT_DELAY = 50
    KEY_REPEAT_INTERVAL = 30  # 더 빠른 반응
    FAST_STEP_SIZE = 2  # 더 빠른 이동
    SLOW_STEP_SIZE = 1
    
    MOTION_SMOOTHNESS = 0.15  # 더 빠른 모션
    LOG_INTERVAL = 100  # 100ms마다 로깅
...
class MotorController:
    ...
    def __init__(self):
        self.motors = [
            ...
            MotorConfig(1, "Shoulder", 512, 634, 512),
            MotorConfig(2, "Upper_Arm", 104, 924, 512),
            MotorConfig(3, "Elbow", 512, 956, 956),
        ...
        self.velocities = [0.0] * len(self.motors)  # 현재 속도
        
        self.presets = self._load_presets()
        self.is_serial_connected = False
        self.arduino = None
        self._init_serial()  # Serial 초기화 주석 처리
    ...
    def _init_serial(self):
        ...
        print(f"{Colors.BLUE}[Serial]{Colors.END} Running in simulation mode (Serial disabled).")
    ...
    def update_positions(self):
        """현재 위치를 목표 위치로 부드럽게 이동 (모션 프로파일 적용)"""
    ...
    def send_command(self):
        ...
        command = f"{','.join(map(str, positions))}*"
...
class UIRenderer:
    ...
    def draw_motor_gauge(self, x, y, width, height, motor_info: dict, motor_index: int):
        ...
        # 상태 아이콘
        state_icon = "●" if motor_info['state'] == MotorState.MOVING else "○"
        state_color = UIColors.SUCCESS_GREEN if motor_info['state'] == MotorState.IDLE else UIColors.ACCENT_BLUE
        state_text = self.font_medium.render(state_icon, True, state_color)
        self.screen.blit(state_text, (x + width - 30, y + 10))
    ...
    def draw_control_panel(self, status_msg: str, is_connected: bool, is_logging: bool):
        ...
        self.screen.blit(status, (panel_x + 20, panel_y + 40))
        
        # 키보드 힌트
        hints = [
            "Q/A: M1  W/S: M2  E/D: M3  R/F: M4  T/G: M5  Y/H: M6",
            "F1-F3: Presets  SHIFT: Slow  L: Toggle Log  ESC: Exit"
        ]
        
        for i, hint in enumerate(hints):
            hint_text = self.font_tiny.render(hint, True, UIColors.TEXT_LIGHT)
            self.screen.blit(hint_text, (panel_x + 20, panel_y + 68 + i * 15))
